...
import se.ltu.thesis.haidar.database.Statistics;
...
public class Main {
	...
	public void QPlaning(String outputPath) {
		...
		((MyQLearning) agent).setLearningPolicy(new MyEpsilonGreedy(
				(QFunction) agent, EPSILON));

		((MyQLearning) agent).setMaxPlaningEpisodeSize(STEPS_IN_EPISODE);
		((MyQLearning) agent)
				.setMaxQChangeForPlanningTerminaiton(DELTA_TERMINATION);
		((MyQLearning) agent).initializeForPlanning(rf, tf, NUM_PLAN_EPISODE);

		GreedyQPolicy mPolicy = ((MyQLearning) agent)
				.planFromState(initialState);
		// AbstractGroundedAction mAction =
		// mPolicy1.getAction(CloudWorld.getPolicyTestState(domain));
		...
		// mPolicy1.evaluateBehavior(env,
		// STEPS_IN_EPISODE).writeToFile(outputPath + "ql_p");
